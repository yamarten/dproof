...
(* target（タクティック）中のpat（名前）をstrに変更 *)
let replace_name pat str target =
  ...
  (* 1つの区切り文字を挟んでpatが2つある場合に繰り返す必要がある（2回で十分？） *)
  let rec repall s =
...
(* by部にhyps、using部にtacを使ったjustificationを生成 *)
(* tac中の名前はenv.renameにしたがって書き換えられる *)
...
(* ゴールが分岐しない = thus,hence,have,thenで済む場合のC-zarステップを生成 *)
(* hence thesisとやってしまうとsimpl的な処理が挟まっていた場合に処理できないのでちゃんと命題を書く *)
...
let wrap_claim root leaf ?name typ body =
  (* 条件の後半はCaseなどそれ自体は名前を付けられない構文を包む時に必要 *)
  (* 本当はもう一つ別の関数を作るべきかもしれない *)
...
(* termが値を表す項だった場合にそれをppにして返す *)
(* 1+1などをいちいちadd,S,Oに分けないための処置 *)
(* ついでにtermがPropかつ環境中に同じ命題が存在する場合はそれで代用させる（少し危険） *)
...
(* TODO:適度な空行 *)
(* typ:（termから得られるより正確な）termの型 rest:evarから変換関数へのマップ *)
let rec pr_term_body root leaf ?name ?typ env evmap rest term =
  ...
  | LetIn (n,b,t,c) ->
    ...
    (* leafはfalseにしても可 1つ余計な仮定を背負い込むだけ *)
    ...
    (* 分岐先でavoidは共有されないため、claimで包むことで名前の衝突を避ける *)
    (* ex. (let x = add in x) (let x = 0 in x) はそれぞれを包まないとxで衝突する *)
...
and pr_lambda root leaf ?name ?typ env evmap rest term =
  ...
  let body root name =
    ...
    (* leafを渡してもよい（haveがthenになって多少の混乱はあるかもしれない） *)
...
(* diffでnat_indなどを使っていると仮定して変換を試み、失敗したら例外で脱出 *)
(* 普通にoptionなどでいいが面倒なので変えていない *)
and pr_ind root leaf ?name ?typ env rest evmap diff =
  (* 通常、*_indの引数の順は「型パラメータ」「命題」「各場合の証明」「帰納法の対象」 *)
  let open Term in
  let open Pcoq in
  let open Str in
  ...
  let pr_branch i s b =
    ...
    (* コンストラクタの引数のうち再帰している（帰納法の仮定になる）ものの数をカウント *)
    let countrec =
      let open Rtree in
  ...
  let body _ _ =
    ...
    str "end induction."
  in
  (* inductionは証明を終わらせるため、仮定にしたければ（root=false）claimで包む *)
  if extra = [||] then wrap_claim false root ?name typ body else
  (* 余った引数には複雑なものが来ないことを期待してby部に書く *)
...
and pr_case root leaf ?name env evmap rest (ci,t,c,bs) =
  ...
  let pr_br n c =
    ...
    (* 分岐した先でいきなりintroしていたり（>0）内部的に全ての引数を取っていない（<0）場合に対応 *)
...
(* 変換後を基準に root:型がゴールに等しいか leaf:直前のステップの結果を使うか *)
(* name:このステップの命題に名前を付けるか *)
...
and pr_path root leaf ?name env (v,diff,(g,e)) next =
  ...
  | Some var, _ ->
    (* （変換前に於ける）残りのステップを証明項で表してしまっていい場合 *)
...
(* 証明の開始（Goalやproof）と終了（endやQed） *)
(* 環境を見て適当にパラメータを追加する（証明の一部のみを変換する場合のため）が、無くしてもいいかも *)
...
(* 選択範囲で生成された証明項を全て繋げてタクティックを使わずに変換 *)
